...
CONTEXTO LINKEDIN/PERFIL: A seção "Dados coletados do LinkedIn" deve ser uma BUSCA DETALHADA. Use os "DADOS ADICIONAIS" e os "RESULTADOS DA WEB" para listar TUDO do LinkedIn: TODAS as experiências (com cargo/título e empresa), TODOS os links adicionados no perfil (website, blog, portfólio, redes), TODOS os projetos (e, em cada projeto, se houver link associado nos resultados, inclua-o), verifique TODOS os destaques em busca de links ou referências a outros sites. Se for achada alguma recomendação, inclua-a no item (h) Recomendações dentro dessa seção.
